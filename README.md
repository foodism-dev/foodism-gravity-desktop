...
Windows 版安装时会检测 Git Bash 或 WSL。如果系统未安装可用 Shell，安装器会提示你安装 Git for Windows（包含 Git Bash）；你也可以跳过，之后在应用内的 Windows 环境检测页重新安装或检测。安装器自动安装 Git for Windows 失败时，会在 `%TEMP%\foodism-gravity-git-install.log` 写入诊断日志，包含 Shell 检测过程、PowerShell/系统信息、下载 URL、HTTP 状态、文件大小和异常消息。
...
| 包 | 版本 | 职责 |
| --- | --- | --- |
| `@proma/electron` | `0.12.70` | Electron 桌面应用 |
