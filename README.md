...
- **Skills & MCP**：每个工作区可以独立配置 Skills、MCP Server 和工作区文件，也可以从自营 Skill 市场安装官方维护的能力包。
...
Windows 版安装时会检测 Git Bash 或 WSL。如果系统未安装可用 Shell，安装器会提示你安装 Git for Windows（包含 Git Bash）；你也可以跳过，之后在应用内的 Windows 环境检测页重新安装或检测。安装器自动安装 Git for Windows 失败时，会通过 `%TEMP%\foodism-gravity-download-git.ps1` 执行下载，并在 `%TEMP%\foodism-gravity-git-install.log` 写入诊断日志，包含 Shell 检测过程、PowerShell/系统信息、下载 URL、HTTP 状态、文件大小和异常消息；下载文件过小时会视为失败，避免把错误页当作安装包执行。
...
Proma 也支持自营 Skill 市场。市场第一版只发布官方维护的 Skill，不包含用户上传、评分、历史版本或推荐位；桌面端通过 `package_sha256` 校验 `.skill` 包，安全解压后安装到当前工作区，并用本地 `.source.json` 记录市场来源。
...
└── apps/
    ...
    └── server/     # Hono + Drizzle 后端服务（认证、Skill 市场）
```
...
| `@proma/server` | `0.1.6` | Hono 后端服务、认证和 Skill 市场 |
...
- `skill-market-service.ts`：Skill 市场列表、下载、sha256 校验、安全解压和工作区安装。
...
`apps/server` 使用 Hono + Drizzle + Postgres。Skill 市场当前使用 `skills`、`skill_tags`、`skill_tag_links` 三张表：一个 `slug` 对应一份当前 `.skill` 包，不维护历史版本，不记录用户安装事件；桌面端通过 `/api/skills`、`/api/skills/:slug` 和 `/api/skills/:slug/download` 获取列表、详情和下载地址。
