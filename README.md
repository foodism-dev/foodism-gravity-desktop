...
Proma 也支持自营 Skill 市场。市场第一版只发布官方维护的 Skill，不包含用户上传、评分、历史版本或推荐位；server 内部发布接口把 `.skill` 包上传到 Cloudflare R2 并把公开下载地址写入 Postgres，桌面端只通过 `package_sha256` 校验下载包，安全解压后安装到当前工作区，并用本地 `.source.json` 记录市场来源。
