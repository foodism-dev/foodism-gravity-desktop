...
describe('Windows NSIS 安装器配置', () => {
  ...
  test('Given Git Bash 安装脚本 When 安装器执行 Then 能检测并拉起 Git for Windows', () => {
    ...
    expect(script).toContain('Var GitInstallerPath')
    expect(script).toContain('Var GitDownloadScriptReady')
    expect(script).toContain('StrCpy $GitInstallerPath "$TEMP\\${GIT_FOR_WINDOWS_EXE}"')
    expect(script).toContain('WriteGitDownloadScript')
    expect(script).toContain('param(')
    expect(script).toContain('[string]$$OutPath')
    expect(script).toContain('GIT_FOR_WINDOWS_MIN_BYTES')
    expect(script).toContain('下载文件过小')
    expect(script).toContain('$GitDownloadScriptReady == "1"')
    expect(script).toContain('-File "$GitDownloadScriptPath"')
    expect(script).toContain('-LogPath "$GitInstallLogPath"')
    expect(script).toContain('-OutPath "$GitInstallerPath"')
    expect(script).toContain('ExecWait \'"$GitInstallerPath"')
    expect(script).not.toContain('powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$$log =')
    expect(script).not.toContain('StrCpy $0 "$TEMP\\${GIT_FOR_WINDOWS_EXE}"')
    expect(script).not.toContain("FileWrite $3 \"$$out = '$0'")
    expect(script).not.toContain("FileWrite $3 \"$$out = '$GitInstallerPath'")
    expect(script).not.toContain('ExecWait \'"$0"')
